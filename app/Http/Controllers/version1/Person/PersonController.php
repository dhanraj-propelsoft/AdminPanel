...
class PersonController extends Controller
{
        ...
        if ($validator->fails()) {
            ...
            if ($response->status() == 200) {
                ...
                if ($result['type'] == 0) {
                    ...
                    $allDatas = $result['personDatas'];
                    $name = $allDatas['first_name'];
                    return view('entitlement.login', ['name' => $name, 'mobile' => $result['mobileNumber']]);
                } else {
    ...
    public function personConfirmationMobileOTP(Request $request)
    {
        ...
        $data = $response->json();
        $personDatas = isset($data['data']) ? $data['data'] : "";
        if (!empty($personDatas['uid'])) {
            echo json_encode($personDatas['uid']);
        } else {
            return 0;
        }
    }
        ...
        if ($validator->fails()) {
            ...
            if ($response->status() == 200) {
                ...
                if (Session::get('token')) {
                    return redirect()->route('getOrganizationAccount');
                }
    ...
    public function forgotPassword(Request $request)
    {
        ...
        Log::info('PersonController > forgotPassword function Inside.' . json_encode($request));
        $response = Http::post(config('api_base') . 'generateEmailOtp', ['uid' => $request->uid]);
        $data = $response->json();

        Log::info('PersonController > forgotPassword function Return.' . json_encode($data));
        if ($response->status() == 200) {
            if ($data['param']) {
                $params = $data['param'];
                $email = $params['email'];
                ...
                if ($request->type == 1) {
                    ...
                    $emailId = substr($email, 0, 4) . "****" . substr($email, 7, 9);
                    return view('entitlement.email_otp', ['personName' => $personName, 'email' => $emailId,'personEmail'=>$email ,'uid' => $params['uid']]);
    ...
    public function personMobileOtpValidated(Request $request)
    {
        ...
        $response = apiHeaders()->post(config('api_base') . 'mobileOtpValidated', ["otp" => $request->otp, 'uid' => $request->uid]);
        $data = $response->json();
    ...
    public function personToUser(Request $request)
    {
        ...
        $response = apiHeaders()->post(config('api_base') . 'personToUser', ['uid' => $request->uid, 'bloodGroup' => $request->bloodGroup, 'gender' => $request->gender, 'dob' => $request->dob]);
        $data = $response->json();
        Log::info('PersonController > personToUser function Return.' . json_encode($data));
        if ($response->status() == 200) {
            $uid = $data['data'];
            return view('entitlement.passwordPage', ['uid' => $uid]);
        } else {
            return redirect()->back()->withErrors(['error' => ['Inappropriate Submisssion']]);
        }
    }
    public function userCreation(Request $request)
    {
        ...
        if ($response->status() == 200) {
            $datas = $data['data'];
            return view('entitlement.login', ['name' => $datas['personName'], 'mobile' => $datas['mobileNumber']]);
        } else {
            return redirect()->back()->withErrors(['error' => ['Inappropriate Submisssion']]);
        }
    }
    ...
    {
        return view("entitlement.reset_password", ['uid' => $request->uid,'personName'=>$request->personName]);
    }
    ...
    public function personValidate(Request $request)
    {
        ...
        $mobileNo = $result[1];
        $email = $result[2];
        $response = apiHeaders()->post(config('api_base') . 'personMobileOtp', ['uid' => $datas['uid'], 'mobileNumber' => $mobileNo]);
        $data = $response->json();
        if ($response->status() == 200) {
            $result = $data['data'];
            $maskedPhone = substr($result['mobileNumber'], 0, 4) . "****" . substr($result['mobileNumber'], 7, 4);
    ...
    public function userView(Request $request)
    {
        ...
        if ($uid) {
            ...
            if ($response->status() == 200) {
                $result = $data['data'];
                return view("Profiles.userViewPage", ['userDatas' => $result[0]['userDeatils'], 'primaryMobiles' => $result[0]['primaryMobile'], 'primaryEmail' => $result[0]['primaryEmail'], 'userProfile' => $result[0]['profilePic'], 'userGender' => $result[0]['userGender'], 'userBloodGroup' => $result[0]['userBloodGroup'], 'UserAddress' => $result[0]['primaryAddress'], 'UserEducations' => $result[0]['UserEducation'], 'userProfession' => $result[0]['userProfession']]);
            } else {
                return redirect()->back()->withErrors(['error' => ['Inappropriate Submisssion']]);
            }
